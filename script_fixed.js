...
$(document).ready(function() {
    ...
    // ===== FIX 4: applyLanguage - إزالة board.resize() منها لأنه يُسبب إزاحة العنوان =====
    function applyLanguage(lang) {
        currentLang = lang; localStorage.setItem('chessLang', lang);
        // نغير dir على body فقط وليس على html لتجنب إزاحة العناصر الثابتة مثل العنوان
        ...
        // board.resize() فقط إن كانت اللعبة نشطة وليس في اللوبي
        if(board && $('#gameArea').is(':visible')) {
            setTimeout(function(){ board.resize(); }, 50);
        }
    }
    ...
    $('#createBtn').click(async function() { 
        ...
        // ===== FIX 3: عرض النقلات بصيغة PGN الصحيحة من البداية =====
        ...
        activeRoomRef.on('value', function(snap) {
            ...
            // ===== FIX 1 + 2 + 3: استقبال نقلة الخصم مع تصليح كامل =====
            if (d.lastMove && d.status === 'playing' && d.fen !== game.fen()) {
                // FIX 3: استخدام pgn لاستعادة التاريخ الكامل وليس fen فقط
                if (d.pgn) {
                    game.load_pgn(d.pgn);
                } else {
                    game.load(d.fen);
                ...
                let isMyTurn = (game.turn() === myPlayerColor.charAt(0));
                board.draggable(gameStarted && isMyTurn);

                // FIX 3: تحديث سجل النقلات بصيغة PGN الصحيحة
                $('#movesHistory').text(game.pgn());
                var movesBox = document.getElementById("movesHistory");
                movesBox.scrollTop = movesBox.scrollHeight;

                updateCapturedPieces(); updateActiveTimerStyle();

                // FIX 1: مسح مؤشرات الحركة بعد استقبال نقلة الخصم
                clearHighlights();
                selectedSquare = null;

                // FIX 2: تنفيذ الـ premove إن كان دور اللاعب
            ...
            if (d.action && d.action.state === 'accepted') {
                ...
                if (d.action.type === 'draw') { activeRoomRef.update({ status: 'draw_agreed', action: null }); } 
                    ...
                    $('#movesHistory').text(''); clearHighlights(); highlightLastMove(null, null); cancelPremove(); updateCapturedPieces();
    ...
    $('#modalCopyPgnBtn, #copyPgnBtn').click(function() { 
        ...
        if (navigator.clipboard) { 
            navigator.clipboard.writeText(pgnData).then(()=>alert(translations[currentLang].msgCopySuccess)).catch(() => { fallbackCopy(pgnData); }); 
    ...
    function processLocalMove(move) {
        ...
        // FIX 1: مسح المؤشرات بعد اللعب مباشرة
        clearHighlights();
        selectedSquare = null;
    ...
    $(document).on('click', '#board .square-55d63', function() {
        ...
        if (selectedSquare) {
            if (game.turn() === myColorCode) {
                ...
                if (move) { board.position(game.fen(), false); processLocalMove(move); return; }
            } else {
                cancelPremove();
    ...
    function onDrop (source, target) { 
        ...
        if (move === null) { 
            // FIX 1: مسح المؤشرات عند snapback أيضاً
            clearHighlights();
            selectedSquare = null; 
            return 'snapback'; 
        } 
